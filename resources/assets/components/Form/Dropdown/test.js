import *as React from 'react'; import { useIsomorphicLayoutEffect } from 'https://framer.com/m/framer/useIsomorphicLayoutEffect.js@^0.2.0';
let safeResizeObserver = undefined;
try { if (typeof window !== undefined) { safeResizeObserver = window.ResizeObserver; } } catch (e) {// happy linting!
...
export function useMeasuredSize(ref, callback) {
    ...
    useIsomorphicLayoutEffect(() => {
        ...
        return () => { if (!ref.current) return; sharedResizeObserver.unobserve(ref.current); };
    }, []); return size;
}
...
export function useSizeChange(ref, callback) {
    ...
    useIsomorphicLayoutEffect(() => {
        ...
        // match the measured size. Future executions of the callback will
        // trigger renders if the size changes.
        sharedResizeObserver.observeElementWithCallback(ref.current, updateSize); return () => { if (!ref.current) return; sharedResizeObserver.unobserve(ref.current); };
